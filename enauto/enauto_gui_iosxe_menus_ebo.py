...
IOSXE_NETCONF_MENU="""Estás usando IOS XE SANDBOX ALWAYS ON
    ...
    4)  ADD NEW INTERFACE               (edit_config operation="create" interfaces)
    5)  MERGE DESCRIPTION               (edit_config operation="merge" interfaces)
    6)  EDIT IP AND MASK                (edit_config operation="replace" interfaces)
    7)  DELETE INTERFACE                (edit_config operation="delete" interfaces)
    ¿Que opción deseas usar?"""

IOSXE_RESTCONF_MENU="""Estás usando IOS XE SANDBOX ALWAYS ON
    1)  GET HEADER AUTHENTICATION       (CREATE HEADER)
    2)  GET CAPABILITIES                (GET /netconf-state/capabilities)
